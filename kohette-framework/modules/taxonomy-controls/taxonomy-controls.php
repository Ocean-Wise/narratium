    ...
    function fields_html($term, $taxonomy = '' ) {
            ...
            if (isset($term->term_id)) {?>
                ...
                <tr class="form-field">
		                <th scope="row" valign="top">
		        	         <label for="<?php echo esc_attr($this->taxmeta_id);?>"><?php echo esc_html($this->taxmeta_name); ?></label>
                    </th>
		              <td>
            ...
            switch ($this->taxmeta_type) {
                ...
                case 'checkbox':
                    ...
                    <input
                    type="checkbox"
                    style="<?php echo esc_html($this->taxmeta_style) ;?>;max-width:16px;"
                    id="<?php echo esc_attr($this->taxmeta_id) ;?>"
                    name="<?php echo esc_attr($this->taxmeta_id);?>"
                    <?php if ($value) { ?> checked <?php } ?>
                    value="1">

                    <?php echo esc_html($this->taxmeta_label);?>
                ...
                case 'select':
                    ...
                    <select
                    style="<?php echo esc_html($this->taxmeta_style);?>;"
                    id="<?php echo esc_attr($this->taxmeta_id) ;?>"
                    name="<?php echo esc_attr($this->taxmeta_id);?>"
                    >
                        ...
                        ?>
                        <option <?php if ($value == $elem_value) {?>selected<?php } ?> value="<?php echo esc_attr($elem_value);?>"><?php echo esc_html($elem_name);?></option>
                        <?php } ?>
                ...
                case 'select2':
                    ...
                    <select
                    style="min-width:300px;<?php echo esc_html($this->taxmeta_style);?>;"
                    id="<?php echo esc_attr($this->taxmeta_id);?>"
                    name="<?php echo esc_attr($this->taxmeta_id);?>[]"
                    multiple="multiple"
                    >
                        <?php foreach($this->taxmeta_type_vars as $select_option) {?>
                        <option <?php if ($value && in_array( $select_option['value'], $value)) { ;?> selected <?php } ?> value="<?php echo esc_attr($select_option['value']);?>"><?php echo esc_html($select_option['name']);?></option>
                        <?php } ?>
                    </select>
                    ...
                    <script>
                          jQuery(document).ready(function() {
                            ...
                            jQuery("#<?php echo esc_js($this->taxmeta_id);?>").select2();
                ...
                case 'image':
                    ...
                    <div id="upload-field-<?php echo esc_attr($this->taxmeta_id);?>">

                        <div  style="margin-bottom:20px;<?php if (!$value) {?>display:none;<?php } ?>" class="show-on-image">

                            <img alt="image" id="uploaded-image-<?php echo esc_attr($this->taxmeta_id);?>" style="display:block;max-height:160px;max-width:500px;" src="<?php echo esc_url($image_src);?>">

                        </div>

                       <span
                       data-uploader_title="<?php echo esc_attr($this->taxmeta_name);?>"
                       id="upload-button-<?php echo esc_attr($this->taxmeta_id);?>"
                       ...
                       id="remove-button-<?php echo esc_attr($this->taxmeta_id);?>"
                       onclick="jQuery('#upload-field-<?php echo esc_attr($this->taxmeta_id);?> .show-on-image').hide();jQuery('#upload-image-<?php echo esc_attr($this->taxmeta_id);?>').val('');"
                       class="show-on-image button button-secondary">
                        ...
                        <input
                        id="upload-image-<?php echo esc_attr($this->taxmeta_id);?>"
                        type="hidden"
                        id="<?php echo esc_attr($this->taxmeta_id);?>"
                        name="<?php echo esc_attr($this->taxmeta_id);?>"
                    ...
                    var file_frame;
                      ...
                      jQuery('#upload-button-<?php echo esc_attr($this->taxmeta_id);?>').live('click', function( event ){
                        ...
                        file_frame.on( 'select', function() {
                          ...
                          jQuery('#upload-image-<?php echo esc_js($this->taxmeta_id);?>').val(attachment.id);

                          jQuery('#uploaded-image-<?php echo esc_js($this->taxmeta_id);?>').attr('src', attachment.url);

                          jQuery('#upload-field-<?php echo esc_js($this->taxmeta_id);?> .show-on-image').show();
                ...
                default:
                    ...
                    <input
                    type="<?php echo esc_attr($this->taxmeta_type);?>"
                    style="<?php echo esc_html($this->taxmeta_style);?>"
                ...
                ?>
                 ...
                 <?php if ($this->taxmeta_description) {?> <p class="description"><?php echo esc_html($this->taxmeta_description);?></p> <?php } ?>
