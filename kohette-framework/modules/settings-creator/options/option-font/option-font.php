...
function KTT_font_field($option, $current_value) {
  ...
  if(!isset($option->option_type_vars['selector'])) return;
                    ...
                    $font_styles = array(
                                    'normal' => esc_html__('Normal','narratium'),
                                    'italic' => esc_html__('Italic', 'narratium'),
                                    'oblique' => esc_html__('Oblique', 'narratium')
                                    );
                    ...
                    */
                    ...
                    <script>
                        var fonts_<?php echo esc_js($option->option_id);?> = {

                            <?php foreach ($default_fonts as $code => $font) {?>
                                '<?php echo esc_js($code);?>': ['<?php echo implode("','", $font['variants']);?>'],
                            <?php } ?>
                        ...
                        function load_variants<?php echo esc_js($option->option_id);?>(code) {

                            if (!code) return;

                            // variants select
                            $variants_select = jQuery('#variants_<?php echo esc_js($option->option_id);?>');

                            variants = fonts_<?php echo esc_js($option->option_id);?>[code];
                    ...
                    name="<?php echo esc_html($option->option_id);?>[selector]"
                    value="<?php echo esc_html($current['selector']);?>">

                    <input
                    type="hidden"
                    <?php $option->link('load_all_weights');?>
                    name="<?php echo esc_html($option->option_id);?>[load_all_weights]"
                    value="<?php echo esc_html($current['load_all_weights']);?>">
                    ...
                    <?php if (isset($option->option_type_vars['font_size']) && $option->option_type_vars['font_size']) {?>
                        ...
                        name="<?php echo esc_html($option->option_id);?>[css][font_size_unit]"
                        >
                            <option value=""><?php esc_html_e('Unit','narratium');?></option>
                            <?php foreach ($size_units as $unit) {?>
                            <option <?php if ($current['font_size_unit'] == $unit) {?> selected <?php } ?> value="<?php echo esc_html($unit);?>"><?php echo esc_html($unit);?></option>
                            <?php } ?>
                        </select>
                    ...
                    name="<?php echo esc_html($option->option_id);?>[font_family]"
                    onchange="load_variants<?php echo esc_html($option->option_id);?>(this.value)"
                    >
                        <option value=""><?php esc_html_e('Default','narratium');?></option>

                        <optgroup label="<?php esc_html_e('Basic fonts','narratium');?>">
                        <?php foreach($default_fonts as $code => $font) {?>
                        <option <?php echo selected($current['font_family'], $code, false) ?> value="<?php echo esc_html($code);?>"><?php echo esc_html($font['name']);?></option>
                        <?php } ?>
                        </optgroup>

                        <optgroup label="<?php esc_html_e('Google fonts','narratium');?>">
                        <?php foreach($google_fonts as $code => $font) {?>
                        <option <?php echo selected($current['font_family'], $code, false) ?> value="<?php echo esc_html($code);?>"><?php echo esc_html($font['name']);?></option>
                        <?php } ?>
                        </optgroup>
                    ...
                    <select
                    id="variants_<?php echo esc_html($option->option_id);?>"
                    style="height:30px;"
                    <?php $option->link('font_weight');?>
                    name="<?php echo esc_html($option->option_id);?>[font_weight]"
                    >
                        <option value="regular"><?php esc_html_e('Regular', 'narratium');?></option>
                    </select>

                    <script>

                    load_variants<?php echo esc_js($option->option_id);?>('<?php echo esc_js($current['font_family']);?>');
                    jQuery('#variants_<?php echo esc_js($option->option_id);?>').val('<?php echo esc_js($current['font_weight']);?>');
                    ...
                    <select
                    style="height:30px;"
                    name="<?php echo esc_attr($option->option_id);?>[font_style]"
                    >
                        <option value=""><?php esc_html_e('Font style','narratium');?></option>
                        <?php foreach ($font_styles as $code => $name) {?>
                        <option <?php if ($current['font_style'] == $code) {?> selected <?php } ?> value="<?php echo esc_html($code);?>"><?php echo esc_html($name);?></option>
                        <?php } ?>
                    </select>
                    ...
                    name="<?php echo esc_attr($option->option_id);?>[css][line_height]"
                    >
                        <option value=""><?php esc_html_e('Line height','narratium');?></option>
                        <?php foreach ($line_heights as $line) {?>
                        <option <?php echo selected($current['line_height'], (string)$line, false) ?> value="<?php echo esc_html($line);?>"><?php echo esc_html($line);?>em</option>
                        <?php } ?>
                    </select>
